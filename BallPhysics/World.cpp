...
void World::removeBody(Id id)
{
    ...
    if(it->second.inGroup)
    {
        ...
        auto cellRange = getCellRange_(it->second.body, 0.f);
    ...
    //velocityChangers_.erase(id);
}

void World::clear()
{
	bodies_.clear();
	//velocityChangers_.clear();
...
/*
void World::addVelocityChanger(Id bodyId, const Body::VelocityChanger& changer)
{
    auto bodyIt = bodies_.find(bodyId);
    assert(bodyIt != bodies_.end());

    auto result = velocityChangers_.emplace(bodyId, changer);
    assert(result.second);
}
*/
...
{
	/*
    for(const auto& ch : velocityChangers_)
    {
        auto bodyIt = bodies_.find(ch.first);
        assert(bodyIt != bodies_.end());

        bodyIt->second.body.affectVelocity(ch.second);
    }
	*/
...
World::CellRange_ World::getCellRange_(const Body& body, float dt)
{
    CellRange_ result;

    std::pair<Rectangle, Vector> boundary = body.boundingVolume(dt);
    //std::pair<Rectangle, Vector> boundary{};
    //boundary.second += worldSize_ / 2.f;
    ...
    //assert(start.x >= -maxError && start.y >= -maxError && end.x >= -maxError && end.y >= -maxError
    //       && start.x <= worldSize_.x + maxError && start.y <= worldSize_.y + maxError && end.x <= worldSize_.x + maxError && end.y <= worldSize_.y + maxError);
...
void World::fillGroup_(CollisionGroup_& group, float dt)
{
    ...
    for(const auto bodyId : group.bodies)
    {
        ...
        auto cellRange = getCellRange_(body, dt);
