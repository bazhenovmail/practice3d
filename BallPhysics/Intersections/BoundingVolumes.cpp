...
std::pair<Rectangle, Vector> boundingVolume(const Line& line, const ShapelessObject& obj, float maxT)
{
	...
	auto minmaxX = std::minmax_element(allVertices.begin(), allVertices.end(), [](const Vector& v1, const Vector& v2) { return v1.x < v2.x; });
	auto minmaxY = std::minmax_element(allVertices.begin(), allVertices.end(), [](const Vector& v1, const Vector& v2) { return v1.y < v2.y; });
