                      ...
                      std::function<void()> onExit) noexcept
{
	...
	//gameSize_.x = brickSize_.x * level_.width;
	//gameSize_.y = brickSize_.y * level_.height;
	...
	/*
	for (const auto& brickColor : brickColors)
	{
		auto mesh = ui_->getEffectColor().createMesh();
		mesh->load(brickSize_.x - 2, brickSize_.y - 2, 0.f,
				   DirectX::XMVectorGetByIndex(brickColor.second, 0),
				   DirectX::XMVectorGetByIndex(brickColor.second, 1),
				   DirectX::XMVectorGetByIndex(brickColor.second, 2),
				   DirectX::XMVectorGetByIndex(brickColor.second, 3));
		brickMeshes_.emplace(brickColor.first, ui_->getGraphics().addMesh(std::move(mesh)));
	}
	*/
	{
		auto mesh = ui_->getEffectColor().createMesh();
...
void Game::enter()
{
	...
	fieldMeshInstance_ = addMeshInstance_(fieldMesh_, []() { return Vector(); });
...
void Game::addBrick_(unsigned index)
{
	...
	if (field_[index].durability)
	{
		...
		field_[index].graphicsIdBackground = addMeshInstance_(brickBackMesh_, [this, brickId]() {return Vector{ world_->getPosition(brickId) }; });
	}
}
...
void Game::addBrickForeground_(unsigned index)
{
	...
	field_[index].graphicsIdForeground = addMeshInstance_(meshIt->second, [this, index]() {return Vector{ world_->getPosition(field_[index].physicsId) }; });
}
